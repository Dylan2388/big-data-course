# Run command:
# time spark-submit --master yarn --deploy-mode cluster --conf spark.dynamicAllocation.maxExecutors=10 --executor-memory 2G --driver-memory 6G numerical_variables_kristen.py > logfile_lastfm.txt 2>&1 /dev/null
...
# Feature: Duration
# Create schema
schema_duration = StructType(\
    [StructField("track_id", StringType(), True),\
     StructField("duration", FloatType(), True)]\
    )
# Read the text file
df_duration = spark.read.csv("/user/s2733226/project/column_data/duration/part*.csv", header = 'False', schema = schema_duration)
# Remove duplicates
